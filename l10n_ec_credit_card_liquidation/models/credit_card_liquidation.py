...
class AccountCreditCardLiquidation(models.Model):
    ...
    withhold_id = fields.Many2one(
        comodel_name="l10n_ec.withhold", string="Withhold", readonly=True
    )
    ...
    def onchange_retention_data(self):
        ...
        domain = {}
        warning = {}
        auth_model = self.env["l10n_ec.sri.authorization.supplier"]
        issue_date = (
        ...
        if self.document_number and self.authorization_id:
            ...
            # Se pudo verificar el formato del numero
            if document_number:
                # Se ha ingresado un numero
                # que pertenece a la misma autorizacion,
                # se le agrega el padding de la autorizacion
                if (
                    issue_date >= auth.start_date
                    and issue_date <= auth.expiration_date
                    and agency == auth.agency
                    and printer_point == auth.printer_point
                    and seq_number >= auth.first_sequence
                    and seq_number <= auth.last_sequence
                ):
                    self.document_number = "{}-{}-{}".format(
                        auth.agency,
                        auth.printer_point,
                        auth_model.fill_padding(seq_number, auth.padding),
                    )
                    return {"value": value, "domain": domain, "warning": warning}
                else:
                    # En caso de no encontrarse como autorizacion validad
                    # debera buscar la siguiente, como si no la hubiera ingresado
                    self.authorization_id = None
        # Se han ingresado todos los datos menos la autorizacion
        if document_number and not self.authorization_id:
            auth_data = auth_model.get_supplier_authorizations(
                "withhold_purchase",
                self.partner_id.id,
                document_number,
                date=issue_date,
            ...
            if auth_ids:
                self.authorization_id = auth_ids[0]
                if auth_data.get("res_number", ""):
                    self.document_number = auth_data.get("res_number", "")
            # si hay mensajes mostrarlos,
            # lo mas probable es q no hay autorizacion activa
            elif auth_data.get("message", "") and not auth_data.get(
                "multi_auth", False
            ):
                raise UserError(_(auth_data.get("message", "")))
        # Se ha escogido la autorizacion pero no se ha ingresado el numero
        if self.authorization_id and not self.document_number:
            auth = self.authorization_id
            self.document_number = "{}-{}-".format(auth.agency, auth.printer_point)

    def action_done(self):
        retention_model = self.env["l10n_ec.withhold"]
        rline_model = self.env["l10n_ec.withhold.line"]
        RetentionPercentage = self.env["l10n_ec.withhold.line.percent"]
        ...
        for liquidation in self:
            ...
            if not liquidation.no_withhold:
                ret = retention_model.create(liquidation._prepare_witholding_vals())
                if (
                    liquidation.rent_base
                    and liquidation.rent_withhold
                    and liquidation.percentage_ret_rent
                ):
                    line_data = {
                        "percent_id": RetentionPercentage._get_percent(
                            liquidation.percentage_ret_rent, "rent"
                        ).id,
                        "base_amount": liquidation.rent_base,
                        "tax_amount": liquidation.rent_withhold,
                        "base_amount_currency": liquidation.rent_base,
                        "tax_amount_currency": liquidation.rent_withhold,
                        "withhold_id": ret.id,
                        "type": "rent",
                    }
                    rline_model.create(line_data)
                if liquidation.iva_withhold and liquidation.percentage_ret_iva:
                    base_amount = liquidation.iva_withhold / (
                        liquidation.percentage_ret_iva / 100.0
                    )
                    line_data = {
                        "percent_id": RetentionPercentage._get_percent(
                            liquidation.percentage_ret_iva, "iva"
                        ).id,
                        "base_amount": base_amount,
                        "tax_amount": liquidation.iva_withhold,
                        "base_amount_currency": base_amount,
                        "tax_amount_currency": liquidation.iva_withhold,
                        "withhold_id": ret.id,
                        "type": "iva",
                    }
                    rline_model.create(line_data)
